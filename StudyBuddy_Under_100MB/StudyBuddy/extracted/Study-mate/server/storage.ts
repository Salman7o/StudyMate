...
export class MemStorage implements IStorage {
  ...
  private initializeData() {
    // Storage is initialized empty - all users will register through the platform
    return;
  ...
  async searchStudents(filters: Partial<{
    ...
    if (filters.subjects && filters.subjects.length > 0) {
      students = students.filter(student => {
        ...
        // If student doesn't have subjects, include them in results
        if (!student.subjects || student.subjects.length === 0) return true;
        
        return filters.subjects!.some(tutorSubject => 
          student.subjects?.some(studentSubject => {
            ...
            return normalizedStudentSubject === normalizedTutorSubject ||
    ...
    if (filters.subjects && filters.subjects !== "All Subjects") {
      tutorsWithUserData = tutorsWithUserData.filter(profile => 
        profile.subjects.some(subject => 
          subject.toLowerCase() === filters.subjects!.toLowerCase()
        )
      );
...
// Use DatabaseStorage since we want to use the PostgreSQL database
