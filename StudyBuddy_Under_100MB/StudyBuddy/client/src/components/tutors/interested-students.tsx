      ...
      sessions.forEach((session: any) => {
        if (session.studentId && session.subject && tutorSubjects.includes(session.subject)) {
          processedStudents.push({
            ...
            hourlyRate: session.student?.hourlyRate, // Added hourlyRate
          });
        }
      });
    }
      ...
      interestedStudents.forEach((student: any) => {
        ...
        if (!processedStudents.some(s => s.id === student.id)) {
          processedStudents.push({
            ...
            hourlyRate: student.hourlyRate, // Added hourlyRate
          });
        }
      });
    }
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="flex justify-between text-sm mt-2">
                      <div className="text-gray-500">Budget/Hour:</div>
                      <div className="text-gray-900 font-medium">
                        {student.hourlyRate ? `Rs. ${student.hourlyRate.toLocaleString()}` : 'Not specified'}
                      </div>
                    </div>
                  </div>
                </div>
